...
void InvariantChecker::checkInvariants(HasInvariants const* _this, char const* _fn, char const* _file, int _line, bool _pre)
{
    if (!_this->invariants())
    {
      wlog("_pre = ${pre}, invariant failed", ("pre", _pre));
      FC_THROW("invariant failed _pre = \"${pre}\"", ("pre", _pre));
    }
}
#endif
/*
TimerHelper::~TimerHelper()
{
    ...
    if (!m_ms || e > chrono::milliseconds(m_ms))
      ilog ("timer id = ${id}, ${dur} ms", ("id", m_id)("dur", chrono::duration_cast<chrono::milliseconds>(e).count()));
}
*/
int64_t utcTime()
{
